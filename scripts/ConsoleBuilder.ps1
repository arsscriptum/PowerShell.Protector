...
$IconPath = ''
$OutputDir = ''
$UserSpecifiedIconPath = $False
[string]$Configuration='Release'
[bool]$UseResourceEncryption = $True
$ObfuscationLevels = @('none','minimum','normal','aggressive','maximum')
$ObfuscationSelection = 'none'

function ShowError([string]$ErrMsg){
    Write-Output "`n"
    Write-Output "ConsoleBuilder v1.0 - PowerShell Script Compiler and Obfuscator`n"
    Write-Output "Error:  $ErrMsg`n"
}
function Usage(){
    Write-Output "`n"
    Write-Output "ConsoleBuilder v1.0 - PowerShell Script Compiler and Obfuscator`n"
    Write-Output "Usage:  ConsoleBuilder.exe <input script file>"
    Write-Output "        ConsoleBuilder.exe [-a] [-d] [-g] <-i input> <-o output> <-n icon> <-x level>"
    Write-Output "           -a  :  as admin"
    Write-Output "           -d  :  debug mode"
    Write-Output "           -g  :  GUI app"
...
if($ArgsCount -eq 0){
    ...
    For($i = 0 ; $i -lt $ArgsCount ; $i++){
        ...
        $valid = (-not([string]::IsNullOrEmpty($arg)))
        if($valid -eq $False) {continue;}
        [char]$cflag = $arg.ToCharArray()[0] -as [char]
        ...
        elseif($name -eq 'x'){
            $i++
            $value = $args[$i]
            if($ObfuscationLevels.Contains($value)){
                $ObfuscationSelection = $value
            }else{
                $msg = "Error: invalid obfuscation level `n"
                ShowError($msg) 
                Start-Sleep 3
                return
            }
...
if(-not (Test-Path "$ScriptPath") ){
    $msg = "Error: No such file `"$ScriptPath`" `n"
    ShowError($msg) 
    Start-Sleep 3
    return
}
...
function Build-Script{
    ...
    $Null = New-Item -Path $ObjPath -ItemType directory -Force -ErrorAction Ignore
    ...
    $ManifestSource = "$ObjPath\PsRunnerApp.win32manifest"
    $AppSource = "$ObjPath\PsRunnerApp.cs"
    $PackagedTemplates = "$ObjPath\cs.zip"
    $AutomationRef = "$RootPath\References\System.Management.Automation.dll"
    $PresentationFrameworkRef = "$RootPath\References\PresentationFramework.dll"
    $ScriptRes = "$ObjPath\logic.bin"
...
function Invoke-Confuser{
    ...
    try {
        ...
        $ConfuserExPath = "$RootPath\ConfuserEx"
        $ILMergePath = "$RootPath\ILMerge"
        $ILMergeExePath = "$ILMergePath\ILMerge.exe"
        $ConfigPath = "$RootPath\tmp"
        $ConfigFilePath = "$RootPath\tmp\confuser.crproj"
...
function Invoke-ILMerge{
    ...
    try {
        ...
        $ILMergePath = (Resolve-Path "$PSBuilderRoot\ILMerge").Path
        $ILMergeExePath = (Resolve-Path "$ILMergePath\ILMerge.exe").Path
        $Basename = (Get-Item "$ScriptPath").Basename
        $MergedExe = "$InputDir\{0}.exe" -f "Program"
        $LogFile = "$ENV:Temp\ILMerge.log"

        #$f = Get-Frameworks | Where Name -eq $FrameWork
        $f = Get-Frameworks -Latest
        $n = $f.Name
        $p = $f.Path
        $tp = "/targetplatform:$n,`"$p`""

        $ExePath = (Resolve-Path "$InputDir\PsRunnerApp.exe").Path
        $DllPath = (Resolve-Path "$InputDir\PsWrapperLib.dll").Path
      
        $ArgsList = @("$TargetDef","/ndebug", "/log:$LogFile","/copyattrs","$tp", "/out:$MergedExe" ,"$ExePath" ,"$DllPath")
...
if([string]::IsNullOrEmpty($PSBuilderRoot)){
    throw "Cannot find Registry Entry for this app. Did you run Setup ?"
}
